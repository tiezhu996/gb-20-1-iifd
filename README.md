...
- **连堂排课**：
  - 课程管理中可设置「一次连排节数」，自动排课把每周课时拆成该节数的完整连堂课组（如每周 5 节、连排 2 节 → 2+2+1）
  - 同一连堂课组的班级、教师、教室在同一天内连续占住，且不跨上午/下午（每天前 4 节为上午，其余为下午）
  - 课组整体放置：剩余空档放不下整组时不拆开，返回课程、星期和建议节次供教务手动安排
  - 锁定课先占位，连堂不得占用其时间；教室容量与班级/教师/教室课表查看照常
